...
export default function ContratoNuevo() {
    ...
    const createContratoMutation = useMutation({
        ...
        if (status === 409 && typeof msg === "string" && msg.includes("Solapamiento")) {
            setOverlapError({
            message: msg,
    ...
    const nombreProfesor = profDetalles
        ? `${profDetalles.apellido || ""} ${profDetalles.nombre || ""}`.trim()
        : "";
    ...
    return (
        ...
        <Panel className="p-5 space-y-5">
            {loadingProf && (
                <div className="opacity-70">Cargando datos del profesor…</div>
            )}
            {errorProf && (
                <div className="mb-3 text-sm text-red-400">
                    No se pudo obtener los datos del profesor. Verificá que tenga
            ...
            <motion.div
            className="p-3 rounded-lg bg-[#242E38] text-sm"
            ...
            <strong className="text-[#19F124]">
                {nombreProfesor || `ID Persona: ${idPersona}`}
            </strong>
            </motion.div>
                        ...
                        <Field label="Carrera">
                            ...
                            <div className="space-y-4">
                                ...
                                const materiasDeFila = it.id_anio
                                    ? materiasPorAnio[it.id_anio] || []
                                    : [];

                                const tarifaRol =
                                    it.cargo && tarifasProfesor.length
                                    ? tarifasProfesor.find(
                                        (t) => t.codigo_cargo === it.cargo
                                        )
                                    : null;
                                ...
                                return (
                                    <div
                                    key={idx}
                                    className="grid grid-cols-12 gap-2 items-end bg-[#18222f] p-3 rounded-xl"
                                    >
                                    <div className="col-span-4">
                                        ...
                                        <select
                                            className="w-24 px-3 py-2 bg-[#242E38] rounded-xl text-xs"
                                            value={it.id_anio}
                                            onChange={async (e) => {
                                            ...
                                            if (!form.id_carrera || !nuevoAnio) return;

                                            if (!materiasPorAnio[nuevoAnio]) {
                                                try {
                                                const { data } =
                                                ...
                                                setMateriasPorAnio((prev) => ({
                                                    ...prev,
                                                    [nuevoAnio]: Array.isArray(data)
                                                    ? data
                                                    : [],
                                                }));
                                                } catch (err) {
                                        ...
                                        <select
                                            ...
                                            setItems((prev) =>
                                                prev.map((row, i) =>
                                                i === idx
                                                    ? { ...row, id_materia: e.target.value }
                                                    : row
                                                )
                                            )
                                            }
                                            disabled={!it.id_anio || !materiasDeFila.length}
                                        >
                                            <option value="">
                                            {!it.id_anio
                                                ? "Elegí año primero"
                                    ...
                                    <div className="col-span-3">
                                        ...
                                        >
                                        <option value="">Seleccionar…</option>
                                        {tarifasProfesor.length > 0 ? (
                                            tarifasProfesor.map((t) => (
                                            <option
                                                key={t.codigo_cargo}
                                                value={t.codigo_cargo}
                                            >
                                                {t.descripcion} (${t.monto_hora} / h)
                                            </option>
                                            ))
                                        ) : (
                                            <>
                                        ...
                                        <p className="mt-1 text-xs text-[#9fb2c1]">
                                            Tarifa automática: ${tarifaRol.monto_hora} por
                                            hora
                                        </p>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-end col-span-1 gap-2">
                                        ...
                                        className="px-2 py-1 text-xs text-red-300 border cursor-pointer rounded-xl border-red-500/60 hover:bg-red-500/10"
                                        >
                                        X
                                        </button>
                                    </div>
                                ...
                                className="mt-1 px-4 py-2 rounded-xl border border-[#19F124] text-[#19F124] hover:bg-[#19F124] hover:text-[#0D1520] cursor-pointer text-sm"
                                >
                                + Agregar materia / rol
                                </button>
                                ...
                                <div className="text-xs opacity-70">
                                    Primero seleccioná una carrera para poder elegir año y
                                    materias.
                                </div>
                                )}
                            </div>
                    ...
                    <Field label="Actividades adicionales (Coordinador, Investigación, Administrativo, RRHH)">
                        <div className="space-y-4">
                            ...
                            return (
                                ...
                                >
                                <div className="col-span-3">
                                    <label className="block mb-1 text-xs opacity-80">Perfil</label>
                                    <select
                                    className="w-full px-3 py-2 bg-[#242E38] rounded-xl text-sm"
                                    value={it.id_perfil}
                                    onChange={(e) => {
                                        ...
                                        (p) => String(p.id_perfil) === String(id_perfil)
                                        );
                                        const tipoPorDefecto =
                                        perfilSel?.codigo === "COOR"
                                            ? "COORDINACION"
                                            : perfilSel?.codigo === "INVEST"
                                            ? "INVESTIGACION"
                                            : perfilSel?.codigo === "ADMITVO"
                                            ? "ADMINISTRATIVO"
                                            : perfilSel?.codigo === "RRHH"
                                            ? "RRHH"
                                            : "";
                                        ...
                                        prev.map((row, i) =>
                                            ...
                                            ? {
                                                ...row,
                                                id_perfil,
                                                tipo_item: tipoPorDefecto || row.tipo_item,
                                                cargo: "",
                                                }
                                            : row
                                        )
                                    ...
                                    {perfilesNoProfesor.map((p) => (
                                        <option key={p.id_perfil} value={p.id_perfil}>
                                            {p.perfil_nombre || p.nombre} ({p.codigo})
                                        </option>
                                        ))}
                                    </select>
                                </div>
                                ...
                                <div className="col-span-2">
                                    ...
                                    onChange={(e) =>
                                        ...
                                        prev.map((row, i) =>
                                            i === idx
                                            ? { ...row, tipo_item: e.target.value.toUpperCase() }
                                            : row
                                        )
                                        )
                                    }
                                ...
                                <div className="col-span-3">
                                    ...
                                    <input
                                    className="w-full px-3 py-2 bg-[#242E38] rounded-xl text-sm"
                                    placeholder="Ej: Coordinador de la Tecnicatura X"
                                    value={it.descripcion_actividad}
                                    onChange={(e) =>
                                        setOtrosItems((prev) =>
                                        prev.map((row, i) =>
                                            i === idx
                                            ? { ...row, descripcion_actividad: e.target.value }
                                            : row
                                        )
                                        )
                                    }
                                ...
                                <div className="col-span-2">
                                    <label className="block mb-1 text-xs opacity-80">Cargo</label>
                                    <select
                                    className="w-full px-3 py-2 bg-[#242E38] rounded-xl text-sm"
                                    value={it.cargo}
                                    onChange={(e) =>
                                        setOtrosItems((prev) =>
                                        prev.map((row, i) =>
                                            i === idx ? { ...row, cargo: e.target.value } : row
                                        )
                                        )
                                    }
                                    disabled={!it.id_perfil}
                                    >
                                    <option value="">Seleccionar…</option>
                                    {(() => {
                                        const perfilSel = perfiles.find(
                                        ...
                                        <option key={t.codigo_cargo} value={t.codigo_cargo}>
                                            {t.descripcion} (${t.monto_hora} / h)
                                        </option>
                                        ));
                                    })()}
                                ...
                                <div className="col-span-2">
                                    ...
                                    onChange={(e) =>
                                        ...
                                        prev.map((row, i) =>
                                            i === idx
                                            ? { ...row, horas_semanales: e.target.value }
                                            : row
                                        )
                                        )
                                    }
                                ...
                                <div className="flex items-center justify-end col-span-1">
                                    ...
                                    type="button"
                                    onClick={() =>
                                        setOtrosItems((prev) => prev.filter((_, i) => i !== idx))
                                    }
                                    className="px-2 py-1 text-xs text-red-300 border cursor-pointer rounded-xl border-red-500/60 hover:bg-red-500/10"
                                    >
                                    X
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                <div className="px-3 py-2 rounded-xl bg-[#242E38] text-sm">
                    <div className="font-semibold">
                        Total horas semanales: {totalHorasSem || 0} hs
                    </div>
                    <div className="text-xs opacity-70">
                        (Se calcula a partir de todas las actividades del contrato)
                    </div>
                </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
                <button
                    type="button"
                    onClick={() => navigate("/dashboard/contratos")}
                    className="px-4 py-2 rounded-xl border-2 border-[#2B3642] hover:bg-[#1A2430] cursor-pointer transition-all"
                >
                    Cancelar
                </button>
                <SolidBtn
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="text-xs text-white/70">
                        El profesor ya tiene un contrato que se superpone con este
                        rango de fechas.
                    </p>
                    </div>
                </div>
                ...
                <div className="space-y-2 text-sm text:white/80">
                    <p className="opacity-90">Mensaje del sistema:</p>
                    <p className="px-3 py-2 rounded-lg bg-[#1b2430] border border-red-500/30 text-red-200 text-xs">
                    {overlapError.message}
                    </p>

                    {(overlapError.fecha_inicio || overlapError.fecha_fin) && (
                    <div className="mt-3 text-xs text-white/75">
                        <p className="mb-1">
                        Intentaste crear un contrato con este período:
                        </p>
                        <ul className="ml-4 list-disc">
                        <li>
                            <span className="opacity-70">Fecha inicio:</span>{" "}
                            {overlapError.fecha_inicio || "—"}
                        </li>
                        <li>
                            <span className="opacity-70">Fecha fin:</span>{" "}
                    ...
                    Revisá las fechas de inicio y fin, o verificá los contratos
                    existentes del profesor antes de volver a intentar.
                    </p>
                </div>
