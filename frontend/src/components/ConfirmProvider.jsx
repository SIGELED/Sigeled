                            ...
                            >
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        <FiAlertTriangle className={state.tone === "danger" ? "text-red-400" : "text-[#19F124]"} size={22} />
                                        <h3 className="text-xl font-semibold text-white">{state.title}</h3>
                                    </div>
                                    <button onClick={() => close(false)} className="cursor-pointer transition-all p-1 rounded-lg text-white hover:bg-[#1A2430]" aria-label="Cerrar">
                                        <IoClose size={20}/>
                                    </button>
                                </div>

                                {state.description && <p className="mb-4 text-sm text-white/80">{state.description}</p>}
                                ...
                                <div className="flex justify-end gap-2">
                                    ...
                                    <button className={`cursor-pointer px-4 py-2 rounded-xl transition-all font-bold ${confirmBtnCls}`} onClick={() => close(true)}>
                                        {state.confirmText}
                                    </button>
                                </div>
                            </div>
