...
import { domicilioService, domOtrosService, personaBarrioService } from "../services/api";
...
export default function PersonaDomicilios({ idPersona, onClose }) {
    ...
    const handleCrearBarrio = async (e) => {
        ...
        try {
        const { data: nuevo } = await domOtrosService.createBarrio(id_localidad, {
            barrio: barrioNombre,
    ...
    return (
        ...
        <div className="absolute inset-0 flex items-center justify-center p-4">
            ...
            role="dialog"
            aria-modal="true"
            className="w-full max-w-4xl rounded-2xl bg-[#101922] p-6 shadow-xl"
                ...
                <ul className="space-y-2">
                    ...
                    <li key={d.id_domicilio} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-[#0D1520]">
                        ...
                        <div className="text-sm opacity-80">
                            ...
                            {/* Localidad / Departamento administrativo */}
            ...
            {showWizard && (
                ...
                <div className="absolute inset-0 flex items-center justify-center p-4">
                    <div className="w-full max-w-3xl bg-[#101922] rounded-2xl p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-start justify-between mb-4">
                        <h4 className="text-xl font-semibold text-[#19F124]">
                        {step === 1 ? "Paso 1: Seleccioná o creá un barrio" : "Paso 2: Datos del domicilio"}
                        </h4>
                    ...
                    {step === 1 && (
                        ...
                        {showCrearBarrio && (
                            ...
                            <div>
                                ...
                                <input value={barrioManzana} onChange={(e) => setBarrioManzana(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" />
                            </div>
                            <div>
                                <label className="block mb-1 opacity-80">Casa</label>
                                <input value={barrioCasa} onChange={(e) => setBarrioCasa(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" />
                            </div>
                            <div>
                                <label className="block mb-1 opacity-80">Departamento (unidad)</label>
                                <input value={barrioDepto} onChange={(e) => setBarrioDepto(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" />
                            </div>
                            <div>
                                <label className="block mb-1 opacity-80">Piso</label>
                                <input value={barrioPiso} onChange={(e) => setBarrioPiso(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" />
                            </div>

                            <div className="flex justify-end col-span-2 gap-2">
                                <button type="button" onClick={() => setShowCrearBarrio(false)} className="px-3 py-2 rounded-xl border-2 border-[#2B3642] hover:bg-[#1A2430]">
                                Cancelar
                                </button>
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button type="button" onClick={() => setShowWizard(false)} className="px-4 py-2 rounded-xl border-2 border-[#2B3642] hover:bg-[#1A2430]">
                            Cancelar
                            </button>
                            <button
                            type="button"
                    ...
                    {step === 2 && (
                        ...
                        <div className="p-3 rounded-xl bg-[#0D1520] text-sm">
                            ...
                            <div className="font-semibold">
                            {(() => {
                                const sel = personaBarrios.find((b) => String(b.id_dom_barrio) === String(selectedBarrioId));
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <input value={calle} onChange={(e) => setCalle(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" required />
                            </div>
                            <div>
                            <label className="block mb-1 text-sm opacity-80">Altura</label>
                            <input value={altura} onChange={(e) => setAltura(e.target.value)} className="w-full px-3 py-2 bg-[#242E38] rounded-xl" required />
                            </div>
                        </div>

                        <div className="flex justify-between">
                            <button type="button" onClick={() => setStep(1)} className="px-4 py-2 rounded-xl border-2 border-[#2B3642] hover:bg-[#1A2430]">
                            Volver
                            </button>
                            ...
                            <button type="submit" disabled={saving} className="cursor-pointer px-4 py-2 rounded-xl font-bold bg-[#19F124] text-[#101922] disabled:opacity-50">
