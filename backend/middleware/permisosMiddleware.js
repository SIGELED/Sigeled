...
export const verificarPropietario = (req, res, next) => {
    try {
        ...
        // Si el usuario es EMP (4) sólo puede acceder a sus propios datos
        if (req.user && Number(req.user.id_rol) === 4) {
            if (!req.user.id_persona || String(id_persona) !== String(req.user.id_persona)) {
