...
// Buscar o crear barrio (race-safe)
export const findOrCreateBarrio = async (client, { barrio, manzana = null, casa = null, departamento = null, piso = null, id_dom_localidad }) => {
  ...
  const r = await runner.query(sel, [barrio, id_dom_localidad]);
  if (r.rows[0]) return r.rows[0].id_dom_barrio;

  try {
    const ir = await runner.query(ins, [barrio, manzana, casa, departamento, piso, id_dom_localidad]);
    return ir.rows[0].id_dom_barrio;
  } catch (err) {
    if (err.code === '23505') { // unique constraint / race
      const r2 = await runner.query(sel, [barrio, id_dom_localidad]);
...
// Crear domicilio (en transaction)
export const createDomicilioWithBarrio = async ({ id_persona, calle, altura = null, barrioData }) => {
  const client = await db.pool.connect();
  try {
    await client.query('BEGIN');

    // barrioData: { id_dom_localidad, barrio, manzana, casa, departamento, piso }
