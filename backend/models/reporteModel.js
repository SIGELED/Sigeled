...
export const getInformeEscalafonario = async (id_persona) => {
    const query = `
        WITH datos_persona AS (
            ...
            LEFT JOIN personas_domicilio pd ON p.id_persona = pd.id_persona
                        ...
                        'materias', (
                            SELECT json_agg(
                                json_build_object(
                                    ...
                                    'descripcion', m.descripcion_materia
                                )
                            )
        ...
        documentos_persona AS (
            SELECT 
                ...
                json_agg(
                    json_build_object(
                        'id_documento', pd.id_doc,
                        'tipo_documento', td.nombre,
                        'nombre_documento', pd.nombre_doc,
                        'fecha_emision', pd.fecha_emision,
                        'archivo_nombre', a.nombre_original,
                        'estado_verificacion', ev.nombre
                    ) ORDER BY pd.fecha_emision DESC NULLS LAST
                ) as documentos
            FROM personas_doc pd
            LEFT JOIN tipos_doc td ON pd.id_tipo_doc = td.id_tipo_doc
    ...
    const query = `
        WITH stats AS (
            SELECT 
                ...
                (SELECT COUNT(*) FROM personas_doc) as total_documentos,
...
export const getEstadisticasContratosPorPeriodo = async (anio) => {
    const query = `
        SELECT 
            ...
            SUM(cp.remuneracion) as total_remuneracion,
            ROUND(AVG(cp.remuneracion)::numeric, 2) as promedio_remuneracion
        FROM contrato_profesor cp
        WHERE ($1::int IS NULL OR cp.anio = $1)
        GROUP BY cp.anio, cp.id_periodo
        ORDER BY cp.anio DESC, cp.id_periodo ASC
    `;
...
export const getEstadisticasDocumentosPorTipo = async () => {
    const query = `
        ...
        FROM personas_doc pd
        JOIN tipos_doc td ON pd.id_tipo_doc = td.id_tipo_doc
...
export const getDocumentosDigitalizados = async (filtros = {}) => {
    ...
    const query = `
        ...
        FROM (
            ...
            -- Documentos
            SELECT 
                pd.id_doc as id_documento,
                'Documento' as tipo_documento,
                pd.id_persona,
                pd.nombre_doc as nombre_documento,
